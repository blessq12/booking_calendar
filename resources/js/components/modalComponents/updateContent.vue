...
export default {
    ...
    setup(props, { emit }) {
        ...
        const form = ref({
            ...
            client_name: props.modalData?.client_name || "",
            client_phone: props.modalData?.client_phone || "",
            comment: props.modalData?.comment || "",
            start_datetime: props.modalData?.start
                ? new Date(props.modalData.start)
                : new Date(),
            end_datetime: props.modalData?.end
                ? new Date(props.modalData.end)
                : new Date(),
            sauna_id: props.modalData?.sauna_id || appStore.selectedSauna,
            prepayment: Number(props.modalData?.prepayment || 0),
            total_amount: Number(props.modalData?.price || 0),
            payment_type: props.modalData?.type || "cash",
        });
        ...
        const handleSubmit = async () => {
            try {
                console.log("Updating booking:", {
                    url: `/api/bookings/${form.value.id}`,
                    method: "PUT",
                    data: form.value,
                });

                await calendarStore.updateBooking(form.value);
                emit("close");
            } catch (error) {
