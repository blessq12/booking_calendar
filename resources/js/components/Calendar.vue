    ...
    methods: {
        ...
        openBookingModal() {
            ...
            this.bookingForm = {
                ...
                end_datetime: new Date(now.setHours(now.getHours() + 1))
                    .toISOString()
                    .slice(0, 16),
                comment: "",
            };
        ...
        createBooking() {
            this.bookingSchema
                .validate(this.bookingForm, { abortEarly: false })
                .then(() => {
                    const postData = { ...this.bookingForm }; // Объединяем данные формы
                    axios
                        .post("/api/bookings", postData)
                        .then(() => {
        ...
        async deleteBooking() {
            ...
            try {
                const response = await axios.delete(
                    `/api/bookings/${this.selectedEvent.id}`
                );
                ...
                toast.error(
                    "Произошла ошибка при удалении брони: " + error.message
                );
            }
        },
                ...
                if (this.$refs.fullCalendar) {
                    ...
                    events.forEach((event) => {
                        try {
                            ...
                            toast.error(
                                "Ошибка при добавлении события:" + e.message
                            );
                        }
                    });
...
<template>
    <div class="min-h-screen bg-gray-900 p-4 text-gray-100">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <!-- Календарь -->
            <div
                class="bg-gray-800 rounded-xl shadow-2xl overflow-hidden border border-gray-600"
            >
        ...
        >
            <div
                class="w-full max-w-lg transform bg-gray-800 rounded-xl shadow-2xl border border-gray-700 transition-all p-6"
            >
                <div class="flex items-center justify-between mb-6">
                ...
                <p class="text-gray-300">
                    Имя клиента: {{ selectedEvent.extendedProps.client.name }}
                </p>
                <p class="text-gray-300">
                    Телефон:
                    <a
                        href="tel:{{ selectedEvent.extendedProps.client.phone }}"
                        class="text-blue-500"
                        >{{ selectedEvent.extendedProps.client.phone }}</a
                    >
                </p>
                <p class="text-gray-300">
                    Комментарий: {{ selectedEvent.extendedProps.comment }}
                </p>
                <p class="text-gray-300">
                    Начало: {{ selectedEvent.start.toLocaleString() }}
                </p>
                <p class="text-gray-300">
                    Окончание: {{ selectedEvent.end.toLocaleString() }}
                </p>
